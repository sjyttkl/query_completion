...
class BeamItem(object):
        ...
        if type(prev_word) == list:
            ...
            for word in prev_word:
                if type(word) == str:
                    ...
                    self.words.append(word.decode("utf8"))
    ...
    def __str__(self):
        #print(self.words)
        seq_words = " ".join(self.words)
        return "beam {0:.3f}: ".format(self.log_probs) +seq_words
...
class BeamQueue(object):
    ...
    def Eject(self):
        ...
        #print("beam queue eject start   ",self.q.qsize()," q")
        score, _ = self.q.get()
        #print("beam queue eject q.get end bound:  ",self.bound, " score ",score)
        self.bound = -score
        #print("beam queue eject bound  ",self.bound, " score ",score)
        self.size -= 1
        #print("beam queue eject end size  ",self.size)

    def __iter__(self):
        return self

    def __next__(self):
        #print(" beam queue next start ",self.q.qsize())
        #try:
        if not self.q.empty():
            _, item = self.q.get()
            return item
        #print(" beam queue next end ",self.q.qsize())
...
def GetCompletions(prefix, user_id, dayofweek,hourofday,m, branching_factor=8, beam_size=300,
                   stop='</S>'):
    """ Find top completions for a given prefix, user and model."""
    m.Lock(dayofweek,hourofday,user_id)  # pre-compute the adaptive recurrent matrix
    #print("prefix  ",prefix)
    ...
    for i in range(36):
        ...
        #print(current_char ," current_char_p  ",current_char_p,"  prev_hidden:  ",prev_hidden)
