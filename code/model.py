...
class MetaModel(object):
    ...
    def __init__(self, expdir):
        ...
        with self.graph.as_default():
            ...
            self.beam_chars = tf.nn.embedding_lookup(self.char_tensor, self.model.selected,name="beam_chars")

    def Lock(self, dayofweek,hourofday,user_id=0):
        """Locking precomputes the adaptation for a given user."""
        print("MetaModel...   ",user_id)
...
#         lockedBias = np.zeros([3 * 513])
        self.session.run(self.model.decoder_cell.lock_op,
                     ...
                     self.model.hourofday:[hourofday],
#                       self.model.decoder_cell.lockedW:lockedW,
                     self.model.dayofweek:[dayofweek]})
...
class Model(object):
    ...
    def BuildGraph(self, params, training_mode=True, optimizer=None):
        ...
        self.avg_loss = tf.divide(total_loss, self.words_in_batch, name="avg_loss")

        #self.avg_loss = total_loss / self.words_in_batch
        #tf.get_variable("avg_loss", avg_loss)


        if training_mode:
            self.train_op = optimizer.minimize(self.avg_loss,name="train_op")
    ...
    def BuildDecoderGraph(self):
        ...
        self.next_hidden_state = tf.concat([next_c, next_h], 1,name="next_hidden_state")
        ...
        #print(self.selected_p)
        #a2 = tf.get_variable(name='a2', shape=self.selected_p.shape(), initializer=self.selected_p)
